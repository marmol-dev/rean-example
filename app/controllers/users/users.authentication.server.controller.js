    ...
    User.filter(
        ...
        .then(function (doc) {
            ...
            req.login(user, function (err) {
                if (err) {
                    console.error('users.authentication.controller', 'signup', 'req.login', err);
...
exports.signin = function (req, res, next) {
    passport.authenticate('local', function (err, user, info) {
        console.log('users.authentication.server.controller', 'signin', err, user, info);
        if (err || !user) {
            ...
            req.login(user, function (err) {
                if (err) {
                    console.error('users.authentication.controller', 'signin', 'req.login', 'error', err);
                    res.status(400).send(err);
                } else {
                    res.json(user);
                }
